...
interface SummaryProps {
  ...
  onManualEntry: (type: TransactionType) => void;
}

const Summary: React.FC<SummaryProps> = ({ transactions, onManualEntry }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col justify-between">
        ...
        <button 
          onClick={() => onManualEntry(TransactionType.RECEITA)}
          className="mt-6 w-full py-2 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 font-bold rounded-xl hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-all text-sm flex items-center justify-center gap-2"
        >
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col justify-between">
        ...
        <button 
          onClick={() => onManualEntry(TransactionType.DESPESA)}
          className="mt-6 w-full py-2 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 font-bold rounded-xl hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-all text-sm flex items-center justify-center gap-2"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" /></svg>
          Adicionar Despesa
        </button>
